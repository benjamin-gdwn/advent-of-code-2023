...
elfArr.forEach((elf) => {
  // remove all letters
  const b = elf.toLowerCase().replace(/[a-z]/g, '')
  let c;
  // if more than 2 numbers then combine first and last
  if(b.length > 2) {
    // assign c to first and last
    c = b.charAt(0) + b.charAt(b.length -1)
  } else if (b.length < 2){
    // make two letters if only one exists
    c = b.charAt(0) + b.charAt(0)
  }else {
    // assign if only 2 numbers
    c = b;
  }
  // string to number
  const d = parseInt(c)
  // continuously add numbers
  nada += d;
})
